...
def test_blank_allowlist_is_unset_not_malformed(monkeypatch):
    """Whitespace-only is "unset" — the default profile applies — not the
    malformed case below. The denylist is still parsed; the profile, being
    an allowlist, simply takes precedence over it at registration time."""
    monkeypatch.setenv("GARMIN_ENABLED_TOOLS", "  \t")
    monkeypatch.setenv("GARMIN_DISABLED_TOOLS", "get_b")

    enabled, disabled, desc = garmin_mcp._resolve_tool_filters()

    assert enabled == set(_COACHING_PROFILE)
    assert desc == "coaching profile (default)"
...
def test_nonblank_value_naming_nothing_is_rejected():
    """A value like ",," is a broken config, not a request for the default."""
    with pytest.raises(ValueError, match="Invalid GARMIN_ENABLED_TOOLS"):
